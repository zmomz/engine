...
class PositionManagerService:
    ...
    async def create_position_group_from_signal(
        ...
        async for session in self.session_factory():
            ...
            await position_group_repo.create(new_position_group)
    ...
    async def handle_pyramid_continuation(
        ...
        async for session in self.session_factory():
            ...
            await position_group_repo.update(existing_position_group)
            await session.commit()
    ...
    async def handle_exit_signal(self, position_group: PositionGroup):
        ...
        async for session in self.session_factory():
            ...
            order_service = self.order_service_class(session) # Assuming OrderService can be instantiated like this
    ...
    async def update_risk_timer(self, position_group_id: uuid.UUID, risk_config: RiskEngineConfig):
        async for session in self.session_factory():
            ...
            if timer_started:
                ...
                await position_group_repo.update(position_group_id, {"risk_timer_expires": expires_at})
